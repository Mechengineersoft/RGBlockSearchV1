...
export default function EpoxyPage() {
  ...
  const { data: results, isLoading, error } = useQuery<EpoxyResult[]>({
    queryKey: ["/api/epoxy", blockNo, partNo, thickness],
    enabled: blockNo.trim().length > 0,
    queryFn: async () => {
      return getEpoxyData(
        blockNo.trim(),
        partNo.trim() || undefined,
        thickness.trim() || undefined
      );
    }
  });
  ...
  // No header colors needed for epoxy page
    ...
    <div className="min-h-screen bg-background flex flex-col">
      <header className="border-b sticky top-0 bg-background z-10">
        <div className="container mx-auto px-0 h-12 flex items-center justify-between">
          <h1 className="text-xl font-bold pl-2">Epoxy Search</h1>
